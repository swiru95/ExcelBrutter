$xmlFilePath="D:\file2brute\bruteMe.xlsx"
...
$Password = $null
Write-Host "Start looking for password for $xmlFilePath"
While ($true) {
     $Password = Get-NextPassword $Password
     try{
        $workbook=$excel.Workbooks.Open($xmlFilePath,0,0,5,$Password)
        Write-Host "Found password: $Password !!"
        $excel.visible = $true
        ...
        Write-Progress 'Trying: ' ($Password)
     }
}
